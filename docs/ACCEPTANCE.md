...
本文件定义可以被自动化测试或可复现命令证明的验收标准。`[ ]` 表示尚未验收，不等于计划遗漏。M0 只建立标准和测试基础，不会把 M1 之后的行为提前标记为完成。
...
| --- | --- | --- | --- | --- |
| AL-01 | [ ] 未验证 | Provider 只返回文本并以 `stop` 结束 | Agent 流式发布文本并正常完成 | 无工具事件；终止原因为 `complete`；仅一次 Provider 请求 |
| AL-02 | [ ] 未验证 | 第一轮返回一个完整 ToolCall，工具成功，第二轮返回最终文本 | Agent 执行一次工具、追加 ToolResult 后继续 | 事件顺序正确；第二次请求含 assistant ToolCall 与对应 ToolResult |
| AL-03 | [ ] 未验证 | Provider 连续多轮请求工具，最后返回文本 | Agent 按轮次与响应顺序执行并最终完成 | 无遗漏或重复调用；上下文顺序稳定；step 计数正确 |
| AL-04 | [ ] 未验证 | 工具返回可恢复错误 | Agent 将失败 ToolResult 反馈给 Provider 并允许下一轮修正 | `tool_end` 明确失败；循环不因领域错误直接崩溃 |
| AL-05 | [ ] 未验证 | Provider 返回结构化错误或抛出 Provider 异常 | Agent 发布脱敏错误并终止 | 不执行工具；终止原因为 `provider_error`；不伪造 assistant 消息 |
| AL-06 | [ ] 未验证 | 调用方在模型流或工具执行中 abort | 当前操作停止且 Agent 结束 | Provider/Tool 收到同一取消信号；终止原因为 `aborted`；无后续调用 |
| AL-07 | [ ] 未验证 | 继续循环将超过 `maxSteps` | Agent 在边界处停止 | 不多发一次 Provider 请求；不多执行一个工具；终止原因为 `max_steps` |
| AL-08 | [ ] 未验证 | ToolCall 缺 ID/name、参数截断、JSON 无法解析或 schema 不匹配 | Agent 拒绝执行并安全终止 | `Tool.execute` 调用次数为 0；无 `tool_start`；终止原因为 `protocol_error` |
